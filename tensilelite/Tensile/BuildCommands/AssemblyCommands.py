...
def _linkIntoCodeObject(
    ...
    if os.name == "nt":
        ...
        if numObjFiles > maxObjFiles:
            batchedObjFiles = [
                objFiles[i : i + maxObjFiles] for i in range(0, numObjFiles, maxObjFiles)
            ]
...
def buildAssemblyCodeObjectFiles(
    ...
    for arch, archKernels in archKernelMap.items():
        ...
        if globalParameters["LazyLibraryLoading"]:
            objectFiles = [
                str(asmDir / (kernelWriterAssembly.getKernelFileBase(k) + extObj))
            ...
            for kernel in archKernels:
                ...
                if coName:
                    coFileMap[asmDir / (coName + extCoRaw)].append(
                        str(asmDir / (kernelWriterAssembly.getKernelFileBase(kernel) + extObj))
                    )
            ...
            for src, dst in Utils.tqdm(
                (
                    (orgCoFileName(kName), newCoFileName(kName))
                    for kName in map(
                        lambda k: kernelWriterAssembly.getKernelFileBase(k), archKernels
                    )
                ),
                "Copying code objects",
            ):
